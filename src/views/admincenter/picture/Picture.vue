...
import { getPageAndCondition, update } from '@/api/media/picture'
...
function handleNotPubPic(reason: string) {
  picture.value!.state = 3
  picture.value!.reason = reason
  update(picture.value!).then(() => {
...
function notPubPic(row: Picture) {
  picture.value = JSON.parse(JSON.stringify(row))
      ...
      <el-table :data="tableData" style="width: 100%" height="calc(100vh - 3rem)" show-overflow-tooltip>
        ...
        <!-- <el-table-column label="查看图片" width="100">
          <template #default="scope">
            <a :href="scope.row.path ? 'https://www.nkbfg.asia/api/media' + scope.row.path : 'https://www.nkbfg.asia/api/media/picture/default.png'" class="showImage" target="_blank">查看</a>
          </template>
        </el-table-column> -->
