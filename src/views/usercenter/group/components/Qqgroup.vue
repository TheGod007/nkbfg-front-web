...
import { getPageAndCondition, deleteById, update } from '@/api/media/qqgroup'
import { Qqgroup, QqgroupPage } from '@/api/media/type'
import { Page } from '@/api/type'
import QqDialog from './QqDialog.vue'
...
const pId = ref()
const weDialogRef = ref<typeof QqDialog>()
const confirmDialongRef = ref<typeof ConfirmDialog>()
const tableData = ref<Qqgroup[]>()

function handlerResult(res: QqgroupPage) {
...
function handleModify(row: Qqgroup) {
  weDialogRef.value!.qqgroup = JSON.parse(JSON.stringify(row))
...
<template>
  <div class="friendsget">
    <div class="fcontainer">
      <el-row :gutter="0">
        <el-col :span="4">
          <el-button class="uploadbutton" @click="uploadImage">上传QQ群</el-button>
          <QqDialog ref="weDialogRef" @save="pageQuery" />
            ...
            <a
              :href="
                scope.row.path
                  ...
                  : 'https://www.nkbfg.asia/api/media/qqgroup/default.png'
              "
              class="showImage"
              target="_blank"
            >
