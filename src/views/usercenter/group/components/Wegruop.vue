...
import { getPageAndCondition, deleteById, update } from '@/api/media/wegroup'
import { Wegroup, WegroupPage } from '@/api/media/type'
import { Page } from '@/api/type'
import WeDialog from './WeDialog.vue'
...
const weDialogRef = ref<typeof WeDialog>()
const confirmDialongRef = ref<typeof ConfirmDialog>()
const tableData = ref<Wegroup[]>()

function handlerResult(res: WegroupPage) {
...
function handleModify(row: Wegroup) {
  weDialogRef.value!.wegroup = JSON.parse(JSON.stringify(row))
  weDialogRef.value!.setDialogVisible(true)
}
...
<template>
  <div class="friendsget">
    <div class="fcontainer">
      <el-row :gutter="0">
        <el-col :span="4">
          <el-button class="uploadbutton" @click="uploadImage">上传微信群</el-button>
          <WeDialog ref="weDialogRef" @save="pageQuery" />
            ...
            <a
              :href="
                scope.row.path
                  ...
                  : 'https://www.nkbfg.asia/api/media/wegroup/default.png'
              "
              class="showImage"
              target="_blank"
            >
