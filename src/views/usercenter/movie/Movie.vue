...
import { getPageAndCondition, deleteById, update } from '@/api/media/movie'
import { Movie, MoviePage } from '@/api/media/type'
...
function handlerResult(res: MoviePage) {
...
function handleModify(row: Movie) {
  myDialogRef.value!.movie = JSON.parse(JSON.stringify(row))
            ...
            <a
              :href="
                scope.row.path
                  ...
                  : 'https://www.nkbfg.asia/api/media/movie/default.png'
              "
              class="showImage"
              target="_blank"
            >
